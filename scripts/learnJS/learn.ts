...
const isStrongPassword = (password: string) => {
    const conditionsRemplis = [false, false, false, false, false, false];
    ...
    conditionsRemplis[5] = regex.test(password);
    const allIsTrue = conditionsRemplis.every((predicate) => predicate);
    if (allIsTrue) {
        return "Valide";
    } else {
        return "Invalide";
